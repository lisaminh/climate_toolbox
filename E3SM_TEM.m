...
function [PSI, divF,Flat,Fp, V_TEM, OMEGA_TEM, W_TEM, lat_ZM,lev,time] = E3SM_TEM(ZM,ZM_nat,lat_ZM,lat,Dates,file_base,years,output_dir)
...
for dd = 1:DD
    ...
    Theta = (1000^kappa)*T.*repmat(lev'.^-kappa,[NN 1 TT]); %here using lev, not pressure (can take top 33 levs where hybm=0, or up to lev=170.744079521582)
    ...
    % TODO: consider implementing the gradient function for langrange interpolation to get unequal spacing (see numpy.gradient)
    Psi = VpThetap./dTheta_bdp;
    dPsidp=[(Psi(:,2,:)-Psi(:,1,:))/(lev(2)-lev(1)), (Psi(:,3:LL,:)-Psi(:,1:LL-2,:))./repmat((lev(3:LL)-lev(2:LL-2))',[MM 1 TT]), (Psi(:,LL,:)-Psi(:,LL-1,:))/(lev(LL)-lev(LL-1))];
    Psi_cos = Psi.*coslat;
    ...
    if dd == DD
        ...
        divF(:,:,(dd-1)*TT_old+1:TTT)=div_F;
        PSI(:,:,(dd-1)*TT_old+1:TTT)=Psi;
    else
        ...
        divF(:,:,(dd-1)*TT+1:dd*TT) = div_F;
        PSI(:,:,(dd-1)*TT+1:dd*TT) = Psi;
    end
...
nccreate(file_out,'divF','Dimensions',{'lat', MM,'lev',LL ,'time',TTT});
nccreate(file_out,'PSI','Dimensions',{'lat', MM,'lev',LL ,'time',TTT});
...
ncwrite(file_out,'divF',divF);
ncwrite(file_out,'PSI',PSI);
